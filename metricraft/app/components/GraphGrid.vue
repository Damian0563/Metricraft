<template>
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mx-4 md:mx-8 p-2 mb-16">
		...
		<ClientOnly>
			<AnimatePresence>
				<motion.div v-for="(entry, i) in enabledMetrics" :key="entry.name" layout :class="spanClassFor(i)"
					...
					<Graph :name="entry.name" :custom="!!entry.customMetrics" :accumulate="!!entry.accumulate"
						...
						:worldData="worldData" @see-details=" handleDetails($event)" @error="errorMessage = $event"
						@timeframe-change="handleTimeframeChange($event)" @metric-updated="loadMetrics" :styling="layout[i]" />
				</motion.div>
			</AnimatePresence>
		</ClientOnly>
	</div>
...
import { clampLayoutAxis, layoutSpanClass, timeframeValueFor, useDisplayCanvas } from '@/composables/helpers';
...
const spanClassFor = (index: number): string => {
	const slot = layout.value[index];
	return slot ? layoutSpanClass[clampLayoutAxis(slot.span)] : '';
};
...
const loadMetrics = async () => {
	...
	if (layout.value.length === 0 || !enabledMetrics.value) return
	let newEnabledMetrics: MetricData[] = []
	layout.value.forEach((entry) => {
		const entryMetrics = enabledMetrics.value!!.find((metric) => metric.name === entry.name)
		if (entryMetrics) newEnabledMetrics.push(entryMetrics)
	})
	enabledMetrics.value = newEnabledMetrics
};
...
watch(() => props.layout, (newLayout: { name: string, span: number, height: number, custom: boolean }[] | undefined) => {
	if (!newLayout) return
