		...
		<div class="max-w-7xl mx-auto mt-8 grid gap-8 lg:grid-cols-2">
			<div class="bg-white rounded-xl shadow-xl p-8 border border-gray-100">
				...
				<div class="space-y-3" v-if="teamUserList.length > 0">
					<ClientOnly>
						<AnimatePresence>
							<motion.div v-for="(user, index) in teamUserList" :key="user.mail" layout
								:initial="{ opacity: 0, height: 0 }" :animate="{ opacity: 1, height: 'auto' }"
								:exit="{ opacity: 0, height: 0, x: 32, transition: { duration: 0.22, ease: 'easeInOut' } }"
								:transition="{ type: 'spring', stiffness: 480, damping: 34, delay: Math.min(index * 0.045, 0.27) }"
					...
					<ClientOnly>
						<AnimatePresence>
							<motion.div v-for="(user, index) in pendingUserList" :key="user.mail" layout
								...
								:exit="{ opacity: 0, height: 0, x: 32, transition: { duration: 0.22, ease: 'easeIn' } }"
								:transition="{ type: 'spring', stiffness: 480, damping: 34, delay: Math.min(index * 0.045, 0.27) }"
								class="flex flex-col gap-3 p-4 bg-gray-50 rounded-lg border border-gray-100 xl:flex-row xl:items-center
							xl:justify-between">
								<div class="flex min-w-0 items-center gap-3">
									<div
										class="h-10 w-10 shrink-0 rounded-full bg-gray-200 flex items-center justify-center text-gray-500">
