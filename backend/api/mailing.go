...
func decisionNotificationCreator(mail string, decision string, appName string) []byte {
	mail = strings.ReplaceAll(strings.ReplaceAll(mail, "\r", ""), "\n", "")
	appName = strings.ReplaceAll(strings.ReplaceAll(appName, "\r", ""), "\n", "")
	decisionText := "denied"
	...
	headers := "From: Metricraft <noreply.metricraft@gmail.com>\r\n" +
		"To: " + mail + "\r\n" +
		"Subject: Your access request for " + appName + " was " + decisionText + "\r\n" +
		"MIME-Version: 1.0\r\n" +
		"Content-Type: text/html; charset=UTF-8\r\n" +
		"Content-Transfer-Encoding: 8bit\r\n" +
		"\r\n"
...
func notifyOfDecision(mail string, decision string, appName string) error {
	apiKey := os.Getenv("GOOGLE_APP_PASSWORD")
	if apiKey == "" {
		return errors.New("GOOGLE_APP_PASSWORD")
	}
	auth := smtp.PlainAuth("", "noreply.metricraft@gmail.com", apiKey, "smtp.gmail.com")
	err := smtp.SendMail("smtp.gmail.com:587", auth, "noreply.metricraft@gmail.com", []string{mail}, decisionNotificationCreator(mail, decision, appName))
	return err
}
