...
func InitDB(ctx context.Context, errChannel chan error) {
	...
	_, err = conn.Exec(ctx, "CREATE TABLE IF NOT EXISTS settings (enabled TEXT, retention INTEGER, appName TEXT)")
	if err != nil {
		errChannel <- err
		return
	}
