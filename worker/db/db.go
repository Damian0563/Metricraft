...
func InitDB(ctx context.Context, errChannel chan error) {
	...
	_, err = conn.Exec(ctx, "CREATE TABLE IF NOT EXISTS settings (enabled TEXT, retention INTEGER, appName TEXT, layout JSONB)")
	if err != nil {
		errChannel <- err
		return
	}
	...
	if _, err = conn.Exec(ctx, "CREATE TABLE IF NOT EXISTS custom_metrics (date TIMESTAMP, metric TEXT PRIMARY KEY NOT NULL)"); err != nil {
		errChannel <- err
		return
	}
